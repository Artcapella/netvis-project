...
├── vtk_output/                   # VTK PolyData files for ParaView (1 per timestep)
...
Scripts are numbered and **must run in order** — each reads the output of the previous step.
...
2. File → Open → select **`vtk_output/abilene_timeseries.pvd`** (this is the only file you need to open)
3. Click **Apply** in the Properties panel
4. In the toolbar dropdown, select **Color by** → `utilization` (or `confidence`, `latency_proxy`, etc.)
5. Optional: Filters → Alphabetical → **Tube** to give links thickness proportional to a metric
6. Use the **animation toolbar** (play/pause/scrub) to step through the 500 timesteps
7. Each timestep represents 5 minutes of real traffic; the timeline shows elapsed minutes

> The `.pvd` file is a lightweight XML index that references the per-timestep `.vtp` files. Keep the `.vtp` files in the same directory.
